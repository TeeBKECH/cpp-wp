 ...
 * @property {string} [type='bullets'] — 'bullets' | 'fraction' | 'progressbar'
 */
/**
...
export function initResponsiveSwiper(container, opts = {}) {
  ...
  function buildPaginationConfig(eff) {
    ...
    const { createInside, ...rest } = typeof paginationOpt === 'object' ? paginationOpt : {}
    ...
    if (paginationOpt === true) {
      ...
      return { config: { ...base, el: internalPaginationEl, clickable: true } }
    }

    const clickable = rest.clickable !== undefined ? rest.clickable : true
    const externalEl = resolveElRelative(rest.el, root)
    if (externalEl) {
      const { el: _el, clickable: _c, ...passThrough } = rest
      return { config: { ...base, el: externalEl, clickable, ...passThrough } }
    }

    if (createInside !== false) {
      internalPaginationEl = document.createElement('div')
      internalPaginationEl.className = 'swiper-pagination'
      root.appendChild(internalPaginationEl)
      const { el: _el, ...passThrough } = rest
      return { config: { ...base, el: internalPaginationEl, clickable, ...passThrough } }
    }
