...
Берётся **только внутренность** элемента **`.uk-margin-medium-top`** (сам класс-обёртка в HTML записи **не** сохраняется — в редактор попадает обычный фрагмент). Удаляются `script`/`style`/iframe, снимаются инлайн-стили и `on*`, все **`span`** разворачиваются (тег убирается), из тела вырезаются **`img`** и **`picture`** (часто битые ссылки на старый сайт). Если заголовок из CSV совпадает с первым `h1` в блоке, дублирующий `h1` убирается.

Первое изображение в блоке **до** вырезания картинок используется для **миниатюры** (если не указан `--no-featured-image`).
