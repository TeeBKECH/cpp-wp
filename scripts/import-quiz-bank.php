 ...
 *
 * Запуск из корня WordPress (рядом с wp-config.php):
 *
 *   wp eval-file wp-content/themes/cpp-courses-theme/../../../scripts/import-quiz-bank.php -- path/to/bank.csv
 *
 * Или скопируйте этот файл на сервер в scripts/ относительно корня WP и:
 *
 *   wp eval-file scripts/import-quiz-bank.php -- scripts/quiz-bank.csv
 *
 * Опции (после --):
 ...
 *   --csv=PATH           путь к CSV (можно вместо первого позиционного аргумента)
 *
 * Формат CSV: первая строка — заголовки. Ожидаемые колонки (имена сопоставляются гибко):
 *   № вопроса | Вопрос | Ответ 1 | Ответ 2 | Ответ 3 | Правильный вариант
 *
 * @package CppCoursesTheme
 */

if (!defined('ABSPATH')) {
    fwrite(STDERR, "Запускайте через WP-CLI: wp eval-file scripts/import-quiz-bank.php -- …\n");
    exit(1);
}
...
if ($csv_path === '' || !is_readable($csv_path)) {
    fwrite(STDERR, "Укажите путь к CSV: wp eval-file scripts/import-quiz-bank.php -- /path/to/bank.csv\n");
    exit(1);
}
