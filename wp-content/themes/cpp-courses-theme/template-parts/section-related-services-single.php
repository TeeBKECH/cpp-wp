<?php
/**
 * "Other services" swiper block for service single.
 *
...
$rel_title = function_exists('get_field') ? (string) get_field('cpp_svc_related_title') : __('Другие услуги', 'cpp-courses-theme');
if ($rel_title === '') {
    $rel_title = __('Другие услуги', 'cpp-courses-theme');
}
$rel_subtitle = function_exists('get_field') ? (string) get_field('cpp_svc_related_subtitle') : '';
$rel_count = function_exists('get_field') ? (int) get_field('cpp_svc_related_count') : 6;
if ($rel_count < 1) {
    $rel_count = 6;
}
if ($rel_count > 12) {
            ...
            <div class="section_footer section_footer--other-services">
                <?php if ($archive) : ?>
                    <a class="button button--filled button--md" href="<?php echo esc_url($archive); ?>">
                        <span class="button_text"><?php esc_html_e('Все услуги', 'cpp-courses-theme'); ?></span>
                    </a>
                <?php endif; ?>
                <nav class="swiper_navigation swiper_navigation--services" aria-label="<?php esc_attr_e('Карусель услуг', 'cpp-courses-theme'); ?>">
